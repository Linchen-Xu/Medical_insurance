...
public class MedTreat extends JFrame{
	...
	String[] column = {"name","group","category","hoslevel","standard","ceiling","percentage"};
	String[] data = {"","","","","","",""};
	Integer size;
	/**
	 ...
	 * @throws Exception 
	 */
	public MedTreat(int i) throws Exception {
		
		//System.out.println(1111);
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MedTreat data1 = new MedTreat();
					data1.setTitle("服务待遇计算参数");
	...
	public MedTreat() {
        ...
        try {
			readper("D:\\eclipse files\\java-neon\\Java_design\\files\\medtreat.txt");
		} catch (Exception e1) {
			e1.printStackTrace();
		}
        ...
        saveButton.addActionListener(new ActionListener(){//添加事件  
        	public void actionPerformed(ActionEvent e){  
        		try {
        			save("D:\\eclipse files\\java-neon\\Java_design\\files\\medtreat.txt");
	...
	public void save(String filename) throws Exception{
		...
		writer.flush(); 
		for(int i=0;i<size;++i){
			line = model.getValueAt(i,0)+"_"+model.getValueAt(i,1)+"_"+model.getValueAt(i,2)+"_"+model.getValueAt(i,3)+
				   "_"+model.getValueAt(i,4)+"_"+model.getValueAt(i,5)+"_"+model.getValueAt(i,6);
			writer.write(line);
			writer.newLine();
			writer.flush(); 
		}
